...
В программе ниже только одно подключение останется в пуле после завершения первого блока ```using (var con = new SqlConnection(connectionString))```, т.к. срабатывает правило для ```LoadBalanceTimeout```. Но далее создается **новое** подключение с аналогичными параметрами, поскольку в пуле должно быть как минимум 2 соединения (```MinPoolSize=2```).
```C#
using System.Threading;
using Microsoft.Data.SqlClient;
using static System.Console;
